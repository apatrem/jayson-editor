...
Completed task bodies moved out of [TASKS.md](TASKS.md) on 2026-06-09 during
the `/agentic-workflow:plan` re-plan. The `[x]`/`[skip]` header stubs remain
in TASKS.md (dependency resolution + marker grammar); this file preserves the
full bodies for history. **Do not work tasks from this file.**
