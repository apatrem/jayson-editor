...
**Amended (2026-06-07, [ADR-0020](adr/0020-json-docmodel-supersedes-yaml.md)):** the **on-disk projection** serializes as **JSON**, not YAML. *Wording note:* D-18's "canonical on-disk format" used *canonical* to mean *the one standard disk encoding* — not source-of-truth. Per memo §2, CONTEXT.md, and the code (`file-open.ts`: YAML→Zod→DocModel; `file-save.ts`: DocModel→YAML), the **DocModel is the canonical model and the on-disk file is a Projection**. This change swaps that projection's encoding (reusing `canonicalize()` unchanged); the canonical model does not change. The YAML premises (human-readable diffs, emergency hand-edit) no longer hold — the file is machine-written and WYSIWYG-edited (R4) — and "JSON is in-memory only" is retired (the DocModel is JSON in memory *and* on disk). Markdown/Markdoc were evaluated and rejected as the container. The text below is the original decision.
